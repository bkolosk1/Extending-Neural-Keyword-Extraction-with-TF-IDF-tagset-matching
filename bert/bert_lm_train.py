...
from model.bert_lm import BertModel, TransformerHead
...
def run_model(batch_size, learning_rate, n_ctx, n_head, n_embd, n_layer, adaptive, bpe, masked_lm, classification, bpe_model_path, datasets, lm_corpus_file, transfer_learning, pos_tags, dict_path, rnn, crf, config_id):
    ...
    parser.add_argument('--result_path', type=str, default='bert_lm_results.txt')
...
def train_test(df_train, df_valid, df_test, args, stemmer, sp, folder=None):
    ...
    if args.classification and args.transfer_learning:
        ...
        model.head = TransformerHead(args)
        model.config = args
    ...
    for epoch in range(args.num_epoch):
        ...
        while i < all_steps - cut:
            ...
            if not args.classification:
                ...
                #print(encoder_words, batch_labels)
                if args.POS_tags:
            ...
            loss = model(encoder_words, input_pos=encoder_pos, lm_labels=batch_labels, embeddings=input_embeddings, masked_idx=None)
        ...
        total_loss, total_seq, total_pred, total_true = test(model, val_data, val_pos, val_target, corpus, args, stemmer, valid_keywords, input_embeddings, sp)
    ...
    total_loss, total_seq, total_pred, total_true = test(model, test_data, test_pos, test_target, corpus, args, stemmer, test_keywords, input_embeddings, sp)
    total_loss = total_loss / total_seq
...
def test(model, data, data_pos, target, corpus, args, stemmer, keywords=None, lm_embeddings=None, sp=None):
    ...
    with torch.no_grad():
        ...
        for i in range(0, all_steps - cut, step):
            ...
            if not args.classification:
                loss, logits = model(encoder_words, input_pos=encoder_pos, lm_labels=input_batch_labels, embeddings=input_embeddings, masked_idx=None, test=True)
            else:
                if not args.crf:
                    loss, logits, att_vector = model(encoder_words, input_pos=encoder_pos, lm_labels=input_batch_labels, embeddings=input_embeddings, test=True)
                else:
                    loss, logits, crf_preds, att_vector = model(encoder_words, input_pos=encoder_pos, lm_labels=input_batch_labels, embeddings=input_embeddings, test=True)
...
if __name__ == '__main__':
    ...
    configs = [
        # lm + bpe + rnn
        {'id': 'bpe+rnn_estonian_big',
         'classification': True,
         'adaptive': False,
         'transfer_learning': False,
         'POS_tags': False,
         'bpe': True,
         'bpe_model_path': 'bpe/bpe_estonian_big.model',
         'masked_lm': True,
         'rnn': True,
         'crf': False,
         'datasets': 'estonian',
         'lm_corpus_file': 'estonian_big.json',
         },
    ]
        ...
        if not transfer_learning:
            if dict_exists:
                ...
                if not classification:
                    ...
                    dict_path = 'dictionaries/dict_' + config['id'] + '_' +  dtl + '_' + dbpe + '_' + dpos + '_' + drnn + '_' + dcrf + '.ptb'
        print(dict_path)
        if classification:
            bs = [8]
        else:
                        ...
                        for n_head in n_heads:
                            for n_embd in n_embds:
                                for n_layer in n_layers:
                                    counter += 1
                                    run_model(batch_size, learning_rate, n_ctx, n_head, n_embd, n_layer, adaptive, bpe, masked_lm, classification, bpe_model_path, datasets, lm_corpus_file, transfer_learning, pos_tags, dict_path, rnn, crf, config['id'])
                                    f.write('Parameters:\n')
