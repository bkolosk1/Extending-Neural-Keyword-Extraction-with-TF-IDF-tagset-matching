...
class BiLSTM_CRF(nn.Module):
    ...
    def __init__(self, vocab_size, embedding_dim, hidden_dim, config, num_tags):
        ...
        if self.n_clusters == 0:
            ...
            for i in range(len(cutoff_values) - 1):
                ...
                if i == 0:
                    ...
                    #print("Tail logit: ", tail_logit_i.size())
                    tail_logprob_i = F.log_softmax(tail_logit_i, dim=1)
                    #print('Tail Logprob 2: ', tail_logprob_i.size())


                    logprob_i = head_logprob_i[:, -i] \
                              + tail_logprob_i.gather(1, target_i[:,None]).squeeze(1)

                    #print('Tail Logprob 3: ', logprob_i.size())
...
class TransformerHead(nn.Module):
    ...
    def __init__(self, model_embeddings, config):
        ...
        if self.classification:
            ...
            self.loss_function = CustomNLLLoss() #CrossEntropyLoss()
            self.decoder = nn.Linear(embed_shape[1], 2, bias=True)
            #self.decoder = nn.Linear(embed_shape[1], 3, bias=True)
            self.num_layers = 2

            if self.rnn:
                self.lstm = nn.LSTM(embed_shape[1], embed_shape[1], dropout=0.3, num_layers=self.num_layers, bidirectional=True)
                self.lstm_dropout = nn.Dropout(0.3)
            elif self.crf:
                #self.crf = BiLSTM_CRF(config.vocab_size, config.n_embd, config.n_embd, config, 3)
                self.crf = BiLSTM_CRF(config.vocab_size, config.n_embd, config.n_embd, config, 2)
    ...
    def forward(self, hidden_state, target, masked_idx, test=False, predict=False):
        ...
        if self.classification:
            if self.crf:
                ...
                #comment line below if iob coding
