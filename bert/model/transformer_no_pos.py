...
class Attention(nn.Module):
    ...
    def forward(self, query, key, value):
        ...
        a, att = self._attn(query, key, value)
...
class EncodingBlock(nn.Module):
    ...
    def forward(self, x):
        ...
        a, att = self.attn(query, key, value)
        # print(a.size(), x.size())
        x = x + a
        m = self.ln_2(x)
        m = self.mlp(m)
...
class TransformerModel(nn.Module):
    ...
    def forward(self, input_ids, input_pos=None, lm_labels=None, embeddings=None, masked_idx=None, test=False,
                predict=False):
        ...
        for block in self.enc_h:
            hidden_states, att = block(hidden_states)
