...
def get_match_result(true_seqs, pred_seqs, do_stem=True, type='exact', lang='english'):
    '''
    ...
    if do_stem:
        true_seqs = [stem_word_list(seq, lang) for seq in true_seqs]
        pred_seqs = [stem_word_list(seq, lang) for seq in pred_seqs]
...
def eval(detected_keywords, gold_standard_keywords, remove_docs_with_no_kw=True, lang='english'):
    all_p_et_5 = []
    all_r_et_5 = []
    all_p_et_10 = []
    all_r_et_10 = []
    ...
    for preds, true in zip(detected_keywords, gold_standard_keywords):
        ...
        match_list = get_match_result(true, preds, lang=lang)
        ...
        if remove_docs_with_no_kw:
            if len(true) == 0 or (len(true) == 1 and true[0] == ['nan']) or (len(true) == 1 and len(true[0]) == 0):
                no_kw += 1
                continue
