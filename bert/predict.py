        ...
        for _, line in df.iterrows():
            ...
            for j, word in enumerate(words):
                ...
                if word in self.dictionary.word2idx:
                    ...
                    idx = len(self.dictionary) + 1 #.word2idx['<unk>']
                if j < max_length:
                    x[i][j] = idx
            i += 1
        return x
...
if __name__ == '__main__':
    ...
    parser.add_argument('--data_path', type=str, default='data/croatian/croatian_test.json', help='path to data in json form')
    parser.add_argument('--bpe_model_path', type=str, default='bpe/bpe_croatian_big.model', help='Path to trained byte pair encoding model')
    parser.add_argument('--trained_classification_model', type=str,
                        default='trained_classification_models/model_5_lm+bpe+rnn_croatian_big_folder_croatian_loss_2.4768645693382894_epoch_8.pt',
                        help='Path to pretrained classification model')
    parser.add_argument('--dict_path', type=str,  default='dictionaries/dict_5_lm+bpe+rnn_croatian_big_adaptive_lm_bpe_nopos_rnn_nocrf.ptb' , help='Path to dictionary')
    parser.add_argument('--result_path', type=str, default='predictions/predictions_croatian_docker.csv')
    ...
    parser.add_argument('--bpe', action='store_false', help='If true, use byte pair encoding.')
    ...
    parser.add_argument('--transfer_learning', action='store_false', help='If true, use a pretrained language model.')
    parser.add_argument('--POS_tags', action='store_true', help='If true, use additional POS tag sequence input')
    parser.add_argument('--classification', action='store_false', help='If true, train a classifier.')
    parser.add_argument('--rnn', action='store_false', help='If true, use a RNN with attention in classification head.')
    parser.add_argument('--crf', action='store_true', help='If true, use a BiLSTM-CRF token classification head.')
    parser.add_argument('--lang', type=str, default='croatian', help='Path to dictionary')
    ...
    df.to_csv(args.result_path, encoding='utf8', sep='\t', index=False)
